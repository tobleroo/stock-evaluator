...
class AlphaStockApiService (){
    ...
    fun fetchAllAlphaData(companySymbol: String, useInDemo: Boolean): CompanyFullData{
        ...
        if(!checkIfCompanyExistsInList(companySymbol)){
            ...
            if (company.overview == null){
                company.overview = fetchAlphaApi.fetchOverviewData(companySymbol, "OVERVIEW", useInDemo)
            }else print("overview exists")
            if (company.incomeData == null){
                company.incomeData = fetchAlphaApi.fetchIncomeData(companySymbol, "INCOME_STATEMENT", useInDemo)
            }else print("income exists")
            if (company.balanceData == null){
                company.balanceData = fetchAlphaApi.fetchBalanceData(companySymbol, "BALANCE_SHEET", useInDemo)
            }else print("baalanbce exists")
            if (company.cashFlowData == null){
                company.cashFlowData = fetchAlphaApi.fetchCashFlowData(companySymbol, "CASH_FLOW", useInDemo)
            }else print("cashflow exists")
            if(company.globalQuote == null){
                company.globalQuote = fetchAlphaApi.fetchGlobalQuote(companySymbol, "GLOBAL_QUOTE", useInDemo)
            }
